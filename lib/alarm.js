...
function stationOptionsFromCatalog(catalog) {
  ...
  catalog.groups.forEach(function (group) {
    ...
    group.stations.forEach(function (station) {
      ...
      options.push({
        value: 'korean_radio_alarm://station/' + station.id,
...
function findStationByUri(catalog, stationUri) {
  ...
  if (target.indexOf('korean_radio_alarm://station/') === 0) {
    target = target.replace('korean_radio_alarm://station/', '');
  }
  ...
  for (var i = 0; i < catalog.groups.length; i++) {
    ...
    for (var j = 0; j < group.stations.length; j++) {
      ...
      if (station.uri === 'korean_radio_alarm://station/' + target || station.id === target) {
        return station;
      }
    }
  }
...
function firstStationUriFromCatalog(catalog) {
  ...
  for (var i = 0; i < catalog.groups.length; i++) {
    ...
    return 'korean_radio_alarm://station/' + group.stations[0].id;
  }
